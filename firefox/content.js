...
knapsack.combiner = function(items, weightfn, valuefn) {
    const _epsilon = 0.01
    const _p = Math.max(...items.map(valuefn))
    ...
    return {
        ...
        ef: (maxweight, step) => {
            const results = [];
            for (let weight = 0; weight <= maxweight; weight += step) {
...
const filterForBest = (maxCoins) => {
    ...
    const elements = cart.items.map((i) => i.element)
